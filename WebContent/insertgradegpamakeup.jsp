...
function editMarks(student_id, marks_id, batch_id){
    var f=document.form;
    f.method="post";
    f.action='edit_gpamakeup.jsp?student_id='+student_id + '&marks_id='+ marks_id + '&batch_id='+ batch_id;
    f.submit();
}
</script>
 ...
 %>
  ...
  <div id="main">
    	...
    	<div id="site_content">
        	...
        	<h1><B>GPA-MAKEUP EXAM GRADE RECORD</B></h1>
            <form action="insertgradeservergpamakeup.jsp?id_batch=<%=request.getParameter("id_batch")%>&id=<%=request.getParameter("id") %>" method="post">
			...
			<p><span>SUBJECT</span><select name="subject"><option value="Select">--Select Subject--</option>
            <%
            	Connection co = null;
		...
		<tr><th width="60" >&nbsp;&nbsp;Code</th><th width="200">&nbsp;&nbsp;Subject Name</th><th width="50">&nbsp;Grade</th><th>&nbsp;Exam Type</th><th></th></tr>
			...
			try{	
				...
				Connection con_sem = DriverManager.getConnection("jdbc:mysql://localhost:8888/onlineresult", "root", "SUD@M@N");
						...
						String sem_query = "select subject.subject_code, subject.subject_name, marks.grade, marks.exam_type, marks.gradepoint, marks.marks_id from subject join course_offered on subject.subject_id = course_offered.subject_id join marks on course_offered.offer_id = marks.offer_id where marks.student_id =" + id + " and marks.exam_type = 'GPA Makeup'";
